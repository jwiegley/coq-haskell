...
Definition vfoldr_with_index
  {B : Type} {n} (f : 'I_n -> A -> B -> B) (b : B) (v : Vec n) : B.
Proof.
  case: n => [//|n] in f v *.
  elim/vecn_rect: v => [x|sz x xs IHxs].
    exact: (f (inord n) x b).
  exact: (f (inord (n - sz.+1)) x IHxs).
Defined.
...
Proof.
  case: n => [//|n] in f v *.
  elim/vecn_rect: v => [x|sz x xs IHxs] in b *.
    exact: (f (inord n) b x).
  exact: (IHxs (f (inord (n - sz.+1)) b x)).
Defined.
...
Example ex_vfoldr_with_index_1 :
  vfoldr_with_index (fun i x rest => (i, x) :: rest) nil
                    (seq_to_vec [:: 1; 2; 3])
    == [:: (inord 0, 1); (inord 1, 2); (inord 2, 3)].
Proof. by rewrite /subn. Qed.

Example ex_vfoldl_with_index_1 :
  vfoldl_with_index (fun i acc x => (i, x) :: acc) nil
                    (seq_to_vec [:: 1; 2; 3])
    == [:: (inord 2, 3); (inord 1, 2); (inord 0, 1)].
Proof. by rewrite /subn. Qed.
